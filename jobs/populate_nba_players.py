...
class PopulateNBAPlayersJob:
    ...
    def run(self):
        ...
        try:
            ...
            for player_data in players:
                existing = db.query(NBAPlayer).filter_by(espn_id=player_data.espn_id).first()
                
                if existing:
                    existing.name = player_data.name
                    existing.team_id = int(player_data.team_id) if player_data.team_id else None
                    ...
                    player = NBAPlayer(
                        ...
                        team_id=int(player_data.team_id) if player_data.team_id else None,
    ...
    def _fetch_all_gamelogs(self, db):
        ...
        players = db.query(NBAPlayer).filter_by(is_active=True).all()
        total = len(players)
        ...
        for idx, player in enumerate(players, 1):
            try:
                ...
                for season in self.SEASONS:
                    ...
                    existing_stats = db.query(NBAPlayerSeasonStats).filter_by(
                    ...
                    for game in games:
                        ...
                        existing_game = db.query(NBAPlayerGameLog).filter_by(
                            player_id=player.id,
    ...
    def _update_season_stats(self, db, player: NBAPlayer, games, season: str):
        ...
        existing = db.query(NBAPlayerSeasonStats).filter_by(
